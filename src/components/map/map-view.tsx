...
export function MapView() {
  ...
  const filteredStories =
    activeMoods.size === 0 ? stories : stories.filter((s) => s.mood && activeMoods.has(s.mood));

  const { points: clusterPoints, getClusterStories } = useStoryClusters(
    filteredStories,
    zoom,
    mapBounds
  );
        ...
        return (
          <div
            ...
            className={`rounded-full transition-all duration-200 ${
              isActive ? "h-0.5 w-5 bg-red-500" : "h-px w-3 bg-red-300/30"
            }`}
          />
        );
      })}
...
function MyLocationButton({ onLocate }: MyLocationButtonProps) {
  ...
  return (
    <div className="relative">
      {showTooltip && (
        <div
          className={`absolute right-11 top-1/2 -translate-y-1/2 mr-2 rounded-lg border border-white/10 bg-neutral-900/90 px-3 py-1.5 text-[12px] tracking-wide backdrop-blur-sm whitespace-nowrap ${
...
function AuthPrompt({ show, onClose }: { show: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* modal */}
      <div
        className="relative animate-fade-in-up space-y-5 rounded-2xl border border-white/10 bg-neutral-900/95 px-8 py-7 text-center shadow-2xl backdrop-blur-sm"
        style={{ animationDuration: "0.3s" }}
      >
